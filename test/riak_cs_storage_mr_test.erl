...
bucket_summary_map_test_() ->
    ...
    CBB = fun(K, Ms) ->
                  ...
                  {proplists:get_value({K, ct}, Res),
                   proplists:get_value({K, by}, Res),
                   proplists:get_value({K, bl}, Res)} end,
    ...
    [
     ...
     ?_assertEqual({1, mb(20),    20}, CBB(ac_de, [MostActive, LessActive])),
     ?_assertEqual({1, mb(10),    10}, CBB(user,  [MostActive, LessActiveMP])),
     ?_assertEqual({3, mb(60)+40, 61}, CBB(ac_de, [MostActive, LessActiveMP])),

     %% Writing MP
     ?_assertEqual({3, mb( 60)+40,  61}, CBB(wr_mp,  [NewWritingMP])),
     ?_assertEqual({3, mb( 60)+40,  61}, CBB(user,   [NewWritingMP])),
     ?_assertEqual({3, mb( 60)+40,  61}, CBB(wr_mp,  [OldWritingMP])),
     ?_assertEqual({3, mb( 60)+40,  61}, CBB(user,   [OldWritingMP])),
     ?_assertEqual({6, mb(120)+80, 122}, CBB(wr_mp,  [NewWritingMP, OldWritingMP])),
     ?_assertEqual({6, mb(120)+80, 122}, CBB(user,   [NewWritingMP, OldWritingMP])),
     ?_assertEqual({0,          0,   0}, CBB(wr_new, [NewWritingMP, OldWritingMP])),
     ?_assertEqual({0,          0,   0}, CBB(wr_old, [NewWritingMP, OldWritingMP])),

     %% Writing non-MP
     ?_assertEqual({1, mb(10),    10}, CBB(wr_new, [NewWriting])),
     ?_assertEqual({1, mb(10),    10}, CBB(wr_old, [OldWriting])),
     ?_assertEqual({1, mb(10),    10}, CBB(wr_new, [NewWriting, OldWriting])),
     ?_assertEqual({1, mb(10),    10}, CBB(wr_old, [NewWriting, OldWriting])),
     %% PD
     ?_assertEqual({1, mb(10),    10}, CBB(pd_new, [NewPd])),
     ?_assertEqual({1, mb(10),    10}, CBB(pd_old, [OldPd])),
     ?_assertEqual({3, mb(60)+40, 61}, CBB(pd_new, [NewPdMP])),
     ?_assertEqual({3, mb(60)+40, 61}, CBB(pd_old, [OldPdMP])),
     %% SD
     ?_assertEqual({1, mb(10),    10}, CBB(sd_new, [NewSd])),
     ?_assertEqual({1, mb(10),    10}, CBB(sd_old, [OldSd])),
     ?_assertEqual({3, mb(60)+40, 61}, CBB(sd_new, [NewSdMP])),
     ?_assertEqual({3, mb(60)+40, 61}, CBB(sd_old, [OldSdMP])),

     %% Combination
     ?_assertEqual({4, mb(70)+40,   71}, CBB(user,   Combination)),
     ?_assertEqual({3, mb(60)+40,   61}, CBB(ac_de,  Combination)),
     ?_assertEqual({3, mb(60)+40,   61}, CBB(wr_mp,  Combination)),
     ?_assertEqual({1, mb(10),      10}, CBB(wr_new, Combination)),
     ?_assertEqual({2, mb(20),      20}, CBB(pd_new, Combination)),
     ?_assertEqual({6, mb(120)+80, 122}, CBB(pd_old, Combination)),
     ?_assertEqual({4, mb(70)+40,   71}, CBB(sd_new, Combination)),
     ?_assertEqual({4, mb(70)+40,   71}, CBB(sd_old, Combination))
    ].
