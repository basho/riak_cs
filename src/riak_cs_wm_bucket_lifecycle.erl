...
content_types_provided(RD, Ctx) ->
    {[{"text/xml", to_xml}], RD, Ctx}.

-spec content_types_accepted(#wm_reqdata{}, #context{}) ->
                                    {[{string(), atom()}], #wm_reqdata{}, #context{}}.
content_types_accepted(RD, Ctx) ->
    {[{undefined, accept_body}, {"text/xml", accept_body}], RD, Ctx}.
...
accept_body(RD, Ctx=#context{user=User,
    ...
    XMLString = unicode:characters_to_list(wrq:req_body(RD)),
    case riak_cs_s3_lifecycle:from_xml(XMLString) of
        {ok, _Lifecycle} ->
            lager:debug("Uploaded lifecycle: ~p", [_Lifecycle]),
            %% TODO: validate the lifecycle values here
            case riak_cs_bucket:set_bucket_lifecycle(User, UserObj, Bucket, XMLString, RcPid) of
                ok ->
                    {{halt, 200}, RD, Ctx};
                {error, Reason} ->
                    riak_cs_s3_response:api_error(Reason, RD, Ctx)
