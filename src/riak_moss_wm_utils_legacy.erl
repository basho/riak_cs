...
-module(riak_moss_wm_utils_legacy).
...
ensure_doc(Ctx=#key_context{get_fsm_pid=undefined, bucket=Bucket, key=Key}) ->
    ...
    {ok, Pid} = riak_moss_get_fsm_sup_legacy:start_get_fsm(node(), [Bucket, BinKey]),
    Metadata = riak_moss_get_fsm_legacy:get_metadata(Pid),
    Ctx#key_context{get_fsm_pid=Pid, doc_metadata=Metadata};
ensure_doc(Ctx) -> Ctx.

streaming_get(FsmPid) ->
    case riak_moss_get_fsm_legacy:get_next_chunk(FsmPid) of
        {done, Chunk} ->
            {Chunk, done};
        {chunk, Chunk} ->
