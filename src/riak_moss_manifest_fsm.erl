...
get_and_update(RiakcPid, Manifest, Bucket, Key) ->
    ...
    ObjectToWrite = case get_manifests(RiakcPid, Bucket, Key) of
        {ok, RiakObject, Manifests} ->
            ...
            OverrittenMarkedAsPendingDelete = riak_moss_manifest:mark_overwritten(NewManiAdded),
            riakc_obj:update_value(RiakObject, term_to_binary(OverrittenMarkedAsPendingDelete));
        {error, notfound} ->
            ManifestBucket = riak_moss_utils:to_bucket_name(objects, Bucket),
