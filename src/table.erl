...
create_table(Spec, Rows, Length, []) ->
    FirstTwoRows = [titles(Spec), vertical_border(Length)],
    create_table(Spec, Rows, Length, FirstTwoRows);
...
create_table(Spec, [Row | Rows], Length, IoList) ->
    Zipped = lists:zip(Spec, Row),
    create_table(Spec, Rows, Length, [row(Zipped) | IoList]).
...
-spec row(list({list(), list()})) -> iolist().
row(_) ->
    "".
...
align(Str, Size) when length(Str) =:= Size ->
    Str ++ " |";
align(Str, Size) when (((Size - length(Str))/2) rem 2) =:= 0 ->
    Spaces = (Size - length(Str)) div 2,
    spaces(Spaces) ++ Str ++ spaces(Spaces) ++ " |";
align(Str, Size) ->
    Spaces = (Size - length(Str)) div 2,
    spaces(Spaces) ++ Str ++ spaces(Spaces) ++ " |".
