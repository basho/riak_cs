...
handle_user_data_response({error, Reason}) ->
    logger:warning("Error occurred requesting user data from keystone. Reason: ~p",
                   [Reason]),
    undefined.
...
handle_ec2_creds_response({error, Reason}, _) ->
    logger:warning("Error occurred requesting user EC2 credentials from keystone. Reason: ~p",
                   [Reason]),
    {undefined, []}.
