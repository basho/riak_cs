...
-spec malformed_request(#wm_reqdata{}, #rcs_context{}) ->
          {false, #wm_reqdata{}, #rcs_context{}} | {{halt, pos_integer()}, #wm_reqdata{}, #rcs_context{}}.
...
process_post(RD, Ctx = #rcs_context{riak_client = RcPid,
...
-spec delete_resource(#wm_reqdata{}, #rcs_context{}) ->
          {boolean() | {'halt', term()}, #wm_reqdata{}, #rcs_context{}}.
delete_resource(RD, Ctx=#rcs_context{local_context=LocalCtx,
                                     riak_client=RcPid}) ->
...
accept_body(RD, #rcs_context{local_context=LocalCtx0} = Ctx0) ->
    ...
    try
        ...
        Ctx = Ctx0#rcs_context{local_context=LocalCtx},
...
validate_copy_header(RD, #rcs_context{response_module=ResponseMod,
                ...
                case riak_cs_manifest:fetch(ReadRcPid, SrcBucket, SrcKey, SrcVsn) of
                    ...
                    {ok, SrcManifest} ->
                        ...
                        validate_part_size(RD,
                                           Ctx#rcs_context{stats_key=[multipart_upload, put_copy]},
                                           End - Start + 1,
...
validate_part_size(RD, #rcs_context{response_module=ResponseMod} = Ctx,
...
prepare_part_upload(RD, #rcs_context{riak_client = RcPid,
    ...
    Caller = riak_cs_user:to_3tuple(Ctx0#rcs_context.user),
...
to_xml(RD, Ctx = #rcs_context{local_context = #key_context{bucket = Bucket, key = Key, obj_vsn = Vsn},
                              riak_client = RcPid}) ->
    UploadId = base64url:decode(re:replace(wrq:path(RD), ".*/uploads/",
                                           "", [{return, binary}])),
    {UserDisplay, _Canon, UserKeyId} = User =
        riak_cs_user:to_3tuple(Ctx#rcs_context.user),
...
finalize_request(RD, Ctx=#rcs_context{local_context=LocalCtx,
    ...
    Caller = riak_cs_user:to_3tuple(Ctx#rcs_context.user),
...
maybe_copy_part(PutPid,
                ...
                ReadRcPid,
                RD, #rcs_context{riak_client=RcPid,
    ...
    case riak_cs_copy_object:test_condition_and_permission(ReadRcPid, SrcManifest, RD, Ctx) of
        ...
        Error ->
            ...
            %% ResponseMod:api_error(Error, RD, Ctx#rcs_context{local_context=LocalCtx})
            Error
    end.
