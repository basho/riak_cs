...
generate_etag(RD, Ctx=#rcs_context{local_context=LocalCtx}) ->
    Mfst = LocalCtx#key_context.manifest,
...
last_modified(RD, Ctx=#rcs_context{local_context=LocalCtx}) ->
    Mfst = LocalCtx#key_context.manifest,
...
produce_body(RD, Ctx=#rcs_context{rc_pool=RcPool,
                                  riak_client=RcPid,
    ...
    {NewCtx, StreamBody} =
        case NoBody of
            ...
            false ->
                ...
                {Ctx#rcs_context{auto_rc_close=false, stats_key=no_stats},
                 {<<>>, fun() ->
...
delete_resource(RD, Ctx=#rcs_context{local_context=LocalCtx, riak_client=RcPid}) ->
    #key_context{bucket=Bucket,
                 key = Key,
                 obj_vsn = ObjVsn,
                 get_fsm_pid = GetFsmPid} = LocalCtx,
    BFile_str = bfile_str(Bucket, Key, ObjVsn),
    UserName = riak_cs_wm_utils:extract_name(Ctx#rcs_context.user),
...
content_types_accepted(CT, RD, Ctx=#rcs_context{local_context=LocalCtx0}) ->
...
-spec accept_body(#wm_reqdata{}, #rcs_context{}) ->
          {{halt, integer()}, #wm_reqdata{}, #rcs_context{}}.
accept_body(RD, Ctx=#rcs_context{riak_client=RcPid,
                                 local_context=LocalCtx,
                                 response_module=ResponseMod})
...
accept_body(RD, #rcs_context{response_module=ResponseMod} = Ctx) ->
    case riak_cs_copy_object:get_copy_source(RD) of
        ...
        {SrcBucket, SrcKey, SrcObjVsn} ->
            handle_copy_put(RD, Ctx#rcs_context{stats_key=[object, put_copy]},
                            SrcBucket, SrcKey, SrcObjVsn)
    end.

-spec handle_normal_put(#wm_reqdata{}, #rcs_context{}) ->
    {{halt, integer()}, #wm_reqdata{}, #rcs_context{}}.
handle_normal_put(RD, Ctx0) ->
    #rcs_context{local_context=LocalCtx,
                 user=User,
                 acl=ACL,
    ...
    Ctx1 = Ctx0#rcs_context{local_context = LocalCtx#key_context{obj_vsn = EventualVsn}},
...
-spec handle_copy_put(#wm_reqdata{}, #rcs_context{}, binary(), binary(), binary()) ->
          {boolean()|{halt, integer()}, #wm_reqdata{}, #rcs_context{}}.
...
-spec accept_streambody(#wm_reqdata{}, #rcs_context{}, pid(), term()) -> {{halt, integer()}, #wm_reqdata{}, #rcs_context{}}.
accept_streambody(RD,
                  Ctx=#rcs_context{local_context=_LocalCtx=#key_context{size=0}},
                  Pid,
                  {_Data, _Next}) ->
...
-spec finalize_request(#wm_reqdata{}, #rcs_context{}, pid()) -> {{halt, 200}, #wm_reqdata{}, #rcs_context{}}.
finalize_request(RD,
                 Ctx=#rcs_context{local_context = LocalCtx,
                                  response_module = ResponseMod,
                                  user = User,
...
check_0length_metadata_update(Length, RD, Ctx=#rcs_context{local_context=LocalCtx}) ->
    ...
    case (not is_atom(LocalCtx#key_context.manifest) andalso
        ...
        true ->
            ...
            {true, RD, Ctx#rcs_context{stats_key=[object, put_copy],
