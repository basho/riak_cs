...
rewrite_path_test() ->
    ...
    rstrt(),
    rewrite_header(riak_cs_aws_s3_rewrite),
    rewrite_path(riak_cs_aws_s3_rewrite),
    strict_rewrite_path(riak_cs_aws_s3_rewrite).

rstrt() ->
    ?assertEqual("foo." ++ ?S3_ROOT_HOST,
                 riak_cs_aws_s3_rewrite:bucket_from_host(
                   "foo." ++ ?S3_ROOT_HOST ++ "." ++ ?S3_ROOT_HOST)).
...
rewrite_path(Mod) ->
    ...
    equal_paths("/buckets/testbucket/objects",
                rewrite_with(Mod, 'GET', headers([]),
                             "/testbucket")),
    equal_paths("/buckets/testbucket/objects",
                rewrite_with(Mod, 'GET', headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/")),
    ...
    equal_paths("/buckets/testbucket",
                rewrite_with(Mod, 'HEAD', headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/")),
    equal_paths("/buckets/testbucket",
                rewrite_with(Mod, 'PUT', headers([]),
                             "/testbucket")),
    equal_paths("/buckets/testbucket",
                rewrite_with(Mod, 'PUT', headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/")),
    equal_paths("/buckets/testbucket",
                rewrite_with(Mod, 'DELETE', headers([]),
                             "/testbucket")),
    equal_paths("/buckets/testbucket",
                rewrite_with(Mod, 'DELETE', headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/")),
    equal_paths("/buckets/testbucket/acl",
                rewrite_with(Mod, headers([]), "/testbucket?acl")),
    equal_paths("/buckets/testbucket/acl",
                rewrite_with(Mod, headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/?acl")),
    ...
    equal_paths("/buckets/testbucket/delete",
                rewrite_with(Mod, 'POST', headers([]),
                             "/testbucket/?delete")),
    equal_paths("/buckets/testbucket/delete",
                rewrite_with(Mod, 'POST', headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/?delete")),
    %% Object Operations
    ...
    equal_paths("/buckets/testbucket/objects/testobject/versions/null",
                rewrite_with(Mod,
                             headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
                             "/testobject")),
    equal_paths("/buckets/testbucket/objects/testdir%2F/versions/null",
                rewrite_with(Mod,
                             headers([{"host", "testbucket." ++ ?S3_ROOT_HOST}]),
